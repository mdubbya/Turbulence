...
{ 
    ...
    {
        public static bool RayCastPath(Vector3 start, Vector3 direction, float width, float length, float rayCastIncrement, SingleUnityLayer targetLayer, out RaycastHit rayCastHitInfo )
        {
            ...
            while((currentDistanceFromStart < width/2) && !hitDetected)
            {
                ...
                if (Physics.Raycast(currentLeftVector, direction, out rayCastHitInfo, length, targetLayer.Mask ))
                {
                    hitDetected = true;
                    break;
                }
                else if (Physics.Raycast(currentRightVector, direction, out rayCastHitInfo, length,targetLayer.Mask))
                {
                    hitDetected = true;
                    break;
                }
